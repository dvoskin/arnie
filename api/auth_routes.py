...
async def link_account(
    ...
    async with AsyncSessionLocal() as db:
        ...
        # If the iOS row already has logs, MERGE them onto the canonical instead
        # of refusing (the old behavior orphaned them — the beta linking blocker).
        # consume_link_code only repoints linked_to_user_id, and the day/food
        # queries are canonical-only, so without this the iOS data would vanish.
        # Resolve the true canonical first — the code owner may itself be linked.
        log_count = (
            await db.execute(
        ...
        if log_count and consumer.id != owner.id:
            ...
            if owner.linked_to_user_id:
                target = await reload_user(db, owner.linked_to_user_id) or owner
            stats = await migrate_user_data(db, consumer, target)
            logger.info(f"Link data-merge user {consumer.id}→{target.id}: {stats}")
