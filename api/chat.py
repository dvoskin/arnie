...
def _display_user_text(row) -> Optional[str]:
    ...
    # Dashboard/card edits store an internal tag ("[edit_food_entry]") as the
    # user side — never render it as a user bubble. Arnie's one-line
    # acknowledgment still shows, so the change is surfaced without any
    # internal-looking text.
