...
async def reset_today_log(db: AsyncSession, user_id: int, user_timezone: str = "UTC") -> bool:
    ...
    # Delete water ROWS too — not just zero the aggregate. Leaving the WaterEntry
    # rows behind meant the next dashboard water edit called recompute_water_total,
    # re-summed the stranded rows, and "resurrected" water the user had cleared.
    await db.execute(delete(WaterEntry).where(WaterEntry.daily_log_id == log.id))
...
async def update_food_entry(
    ...
    """
    Update a food entry, then RECOMPUTE the affected day totals from the entry
    rows (never delta math — recompute can't drift). Returns None if the entry
    ...
    # Apply nutrition changes to the entry (numeric — coerce to float)
    for field in ("calories", "protein", "carbs", "fats", "fiber", "sugar", "sodium"):
        if field in changes and changes[field] is not None:
            setattr(entry, field, float(changes[field]))

    # Non-numeric fields (text + meal metadata) — "fix the sodium" / "that was
    # dinner not lunch" edits used to silently no-op because these weren't applied.
    for field in ("parsed_food_name", "quantity", "meal_type", "meal_time"):
        if field in changes and changes[field] is not None:
