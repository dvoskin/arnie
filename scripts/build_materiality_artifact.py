...
async def build_one(food_name: str) -> dict:
    ...
    records, query_count, failed_queries = await _records_for(
        ...
        art.ROWS_PER_SHAPE)
    if not records:
        return {"food": food_name, "material": False,
    ...
    if failures and not any(a.relationship != ABSTAIN for a in assessments):
        return {"food": food_name, "material": False,
                "reason": f"resolver abstained every batch ({failures})",
    ...
    return {"food": food_name, "material": material, "space": space,
...
async def main() -> int:
    ...
    parser.add_argument("--force", action="store_true",
                        help="write even if provider queries failed")
    ...
    print(f"\n{'identity':<14} {'material':<9} {'space'}")
    print("-" * 76)
    for r in results:
        space = ", ".join(f"{k} {v:.0f}" if v else f"{k} —"
                          for k, v in sorted((r.get('space') or {}).items()))
        print(f"{r['food']:<14} {str(r['material']):<9} "
              f"{space or r.get('reason', '')}")
        if not r["material"] and r.get("space"):
            print(f"{'':<24}({r['reason']}; {r['records']} records from "
                  f"{r['queries']} queries)")

    entries = {}
    if args.merge:
        try:
            entries = dict(json.loads(
                art.ARTIFACT_PATH.read_text(encoding="utf-8"))["entries"])
        except Exception:
            entries = {}
    for r in results:
        if r["material"]:
            entries[r["food"]] = {"space": r["space"],
    ...
    kept = sum(1 for r in results if r["material"])
    lost = sum(r.get("failed_queries", 0) for r in results)
    total_q = sum(r.get("queries", 0) for r in results)
    print(f"\n{kept}/{len(results)} identities produced a material space")

    # REFUSE TO WRITE A BUILD THAT RAN AGAINST A DEGRADED PROVIDER. Every
    # failed query is evidence that did not arrive, and evidence that did not
    # arrive reads exactly like evidence that says "immaterial".
    if lost:
        print(f"\n{lost}/{total_q} provider queries FAILED — the missing rows "
              f"are indistinguishable from rows that say 'not material'.",
              file=sys.stderr)
        if not args.force:
            print("Refusing to write. Re-run when the provider is healthy, "
                  "or pass --force if you accept a partial artifact.",
                  file=sys.stderr)
            return 3
