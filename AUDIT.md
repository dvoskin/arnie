...
### 5. SQLite caps you at a single instance  ✅ DONE (code) — cutover pending
...
**Done (commits c84f325, d9910e1, e4c1af1, adeb9ca):** psycopg3 driver pinned (cp314
wheel verified so the 3.14 build won't break); `render.yaml` declares a managed
`arnie-db` + injects `DATABASE_URL` fromDatabase + runs `alembic upgrade head` as the
preDeployCommand. Fresh-start (no SQLite data migrated; old file kept as fallback).
**REMAINING — your action in Render:** confirm the DB `plan` id, apply the blueprint,
deploy. Rollback = revert adeb9ca's DATABASE_URL to the SQLite path.

### 6. Hand-rolled migrations instead of a framework  ✅ DONE (code)
...
**Done:** Alembic owns the schema. Baseline `fa41393154b3` autogenerated from the
models (verified drift-free via `alembic check`). On Postgres `init_db()` no longer
calls create_all/_migrate — Alembic is the only thing touching schema. **How-to for
future schema changes: see `alembic/README`** (autogenerate → review → `alembic check`
→ commit with the model change). The SQLite `_migrate()`/auto-heal path is retained
ONLY for local dev + the in-memory test suite; retiring it once fully on Postgres is
tracked as a follow-up (see "Follow-ups" below).

---

## Follow-ups (opened during the Postgres migration)
- **Retire the SQLite `_migrate()` + auto-heal path** once prod is confirmed stable on
  Postgres. It currently runs only when DATABASE_URL is SQLite (local dev + tests), so
  it's harmless, but it's dead weight long-term. Options: switch local dev + tests to
  run `alembic upgrade head` against an ephemeral SQLite/Postgres so there's a single
  schema path everywhere, then delete `_migrate()`. Low priority, do after a few clean
  weeks on Postgres.
