...
#: When the composer is on it rewrites every one of them from the committed
#: snapshot, the shared persona and the thread — so asking the interpreter for
#: them buys nothing and costs generation time. Measured across four real
#: production responses, `say` alone was 43% of the interpreter's output
#: tokens, and output tokens are what the user waits on.
#:
#: Marked in place rather than kept as separate constants because three of the
#: five spans sit mid-JSON inside the action examples, where a bullet cannot
#: reach. `[[SAY …SAY]]` survives only when the interpreter narrates;
#: `[[MUTE …MUTE]]` only when it does not. Leading commas live INSIDE the
#: span so a muted example is still valid JSON.
#:
#: `follow_up` is deliberately NOT muted: it is a single enum token, not prose,
#: and it is the only one of the three that the fallback path can still act on.
...
_SYSTEM = (
    ...
    "commit). SAY]][[MUTE- MUTE]]"
    'Optional "follow_up":"save_as_regular" ONLY when the meal looks '
    ...
    "SAY]]"
    "[[MUTEYOU DO NOT WRITE THE REPLY. A separate pass speaks to the user, from "
    "the committed numbers and the whole thread. Emit NO \"say\" and NO \"note\" "
    "on any action — a sentence here is thrown away, and the user waits on you "
    "to finish writing it. Your entire output is the decision and the rows.\n"
    "MUTE]]"
    ...
    "tool or database names in [[SAY'say' or SAY]]questions. Natural coach "
    "language only.\n"
...
def _interpreter_system(*, narrate: bool) -> str:
    ...
    before the spans were marked — the escape hatch keeps its prose."""
