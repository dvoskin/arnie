...
_SYSTEM = (
    ...
    "THE SAME CLOSED FIELD VOCABULARY APPLIES HERE - quantity, multiplier, "
    "consumed, prep, extras, identity - and `prep` above is only this "
    "example's field, not the default. A question about whether they "
    "FINISHED something is `consumed`; about toppings or sauces, "
    "`extras`; about one serving versus two, `multiplier`. "
    ...
    "THE FIELD VOCABULARY IS CLOSED AND IS THE SAME WHETHER YOU ASK OR "
    "LOG. Pick the ONE that names what is actually open:\n"
    "  quantity   - how much of the item there was\n"
    "  multiplier - one standard unit or more than one (a single vs "
    "double scoop, one vs two pieces) where the unit itself is known\n"
    "  consumed   - how much of what was SERVED was actually eaten "
    "(finished it, left some, split it)\n"
    "  prep       - how it was cooked, or fat added in cooking\n"
    "  extras     - unstated components on or in it (toppings, sauces, "
    "dressing, add-ons)\n"
    "  identity   - which item, variant, cut or brand it was\n"
    "Use the CLOSEST one; never invent a field name. `assumed` is shown "
...
def _settle_deferred(out: Optional[dict], prior: Optional[dict]) -> Optional[dict]:
    ...
    if not held:
        return out
    # ⛔ P17 closure Phase 1 — CAUSAL PROVENANCE. A call that came from the
    # prior's stash is an EARLIER action being settled on this turn; the
    # scan authority needs to tell it from a fresh statement ("the plan
    # explicitly references the prior operation"), so it is marked on the
    # call's own input — the source every downstream copy is made from.
    for _c in held:
        if isinstance(_c, dict) and isinstance(_c.get("input"), dict):
            _c["input"]["_prior_held"] = True
...
async def _run_untraced(message: str, user, prior: Optional[dict] = None,
    ...
    # ⛔⛔ NO SCAN INSTRUCTION REACHES THIS PROMPT *(P17 closure directive,
    # Phase 1, superseding 9cf29b9)*. A "SCAN ATTACHED — treat this as a fresh
    # statement, not a correction" line was added here after the 2026-08-19
    # 15:57 canary and REMOVED the same evening: it decided binding from the
    # ATTACHMENT, before the typed plan existed, and it would rewrite the
    # unrelated clauses of a mixed turn ("make the chicken 8 oz" while a bar
    # is scanned). The interpreter parses the user's words exactly as it
    # would without a barcode; `core.scan_authority` then decides, from the
    # COMPLETE plan and the user's literal words beside the snapshot's own
    # identity, whether the scan binds. Any future model guidance about scans
    # must be subject-scoped and shipped separately; it is not what makes the
    # safety invariant hold.
    mode = _mode(user)
