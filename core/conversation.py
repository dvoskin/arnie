...
async def run_turn(
    ...
    *,
    ...
    # A log_food / log_exercise call becomes a macro_card / workout_card on the
    # wire — but ONLY when it actually created (or rolled up into) a real DB row.
    # The dispatcher stashes that row id on inp["_entry_id"]; a deduped / no-op
    # call (the model re-fired something already on the board, e.g. a spurious
    # log_food on a NON-food message) never sets it. Gating on _entry_id stops a
    # stale card leaking onto a reply that logged nothing — Danny 2026-06-26,
    # "what is 84.9kg in lbs" surfaced the earlier coffee's macro_card with
    # entry_id=null. The card must mirror a row the user can actually tap/edit.
    # The iOS client renders these inline beneath the text; Telegram/iMessage
    # adapters ignore the field. Payload pulled from the tool_call INPUT — what
    # the LLM said to log — which is what Arnie's reply confirms.
    if tool_calls:
        for tc in tool_calls:
            name = tc.get("name")
            inp = tc.get("input") or {}
