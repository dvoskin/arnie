...
async def test_1_the_fingerprint_and_its_version_are_persisted_at_creation(
    ...
    opened = await open_operation(db, user=user, interpreter_item=item,
                                  ...
                                  cohort="allowlist", capability="chips")
    await db.commit()
...
async def test_4_ownership_user_domain_turn_and_operation_id(db, make_user):
    from core.b1_quantity_operation import (OpenedElsewhere,
                                            _stored_open_result,
                                            open_operation)
    user = await make_user()
...
async def test_5_a_reuse_renders_every_fact_from_the_row(db, make_user):
    ...
    first = await open_operation(db, user=user, interpreter_item=item,
                                 ...
                                 cohort="allowlist", capability="chips")
    await db.commit()
    assert first.created and first.capability == "chips"
    again = await open_operation(db, user=user, interpreter_item=item,
                                 interaction=inter, turn_id=tid, locale="en",
                                 cohort="scan_bound", capability="text_only")
    assert again.reused
    assert (again.locale, again.cohort, again.capability) == ("ru", "allowlist", "chips")
...
async def test_5_the_bound_ask_renders_the_stored_capability(db, make_user,
    ...
    assert first is not None and first.capability == "ios"
    await db.commit()
    again = await open_bound_quantity_ask(db, user=user, item=item,
                                          coverage=coverage, turn_id=tid,
                                          channel="telegram", locale="en")
    assert again is not None
    assert again.capability == "ios", "the retry re-rendered under its own channel"
...
def test_the_decoder_never_coerces_a_falsy_item_or_skips_a_check():
    ...
    # every ownership and version field is READ (structurally: the names
    # appear as loads or constants in the decoder's body)
    src = inspect.getsource(m._stored_open_result)
    code = src.split('"""', 2)[-1]                 # body only, no docstring
    for needle in ("expect_operation_id", "expect_user_id", "expect_turn_id",
                   "DOMAIN", "schema_version", "fingerprint_version"):
        assert needle in code, needle
    raises = [n for n in ast.walk(tree) if isinstance(n, ast.Raise)]
    assert len(raises) >= 8, f"only {len(raises)} refusals in the decoder"
