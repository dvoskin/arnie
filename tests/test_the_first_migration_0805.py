...
async def pg(monkeypatch):
    ...
    async with maker() as s:
        ...
        s.add(DailyLog(user_id=1, date=DAY, total_calories=320))
        await s.commit()
    yield maker
...
async def test_the_shadow_writes_nothing(pg):
    ...
    assert await _rows(pg, FoodEntry) == 0, "the shadow left rows behind"
...
async def test_a_divergence_names_the_field(pg, caplog):
    ...
    assert await _rows(pg, FoodEntry) == 0
...
async def test_a_failure_while_BUILDING_the_shadow_is_contained(pg,
    ...
    async with pg() as s:
        ...
        s.add(FoodEntry(daily_log_id=1, parsed_food_name="after", calories=1))
        await s.commit()
    assert await _rows(pg, FoodEntry) == 1, "the request did not survive"
