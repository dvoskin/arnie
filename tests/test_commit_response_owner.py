...
def test_where_the_card_renders_the_card_owns_the_facts():
    """Not stripped after the fact — never written. The plan knows which facts
    the card shows before anything is authored."""
    assert fallback(_plan(card=True)).strip() == ""
...
def test_a_commit_with_no_snapshot_has_nothing_to_describe():
    ...
    assert fallback(_plan(card=False, snapshot=False)) == ""
    assert fallback(_plan(card=True, snapshot=False)) == ""
    # ...and with a snapshot, the no-card surface is not silent.
    assert fallback(_plan(card=False)) != ""
