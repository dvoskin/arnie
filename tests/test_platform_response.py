...
the model keeps breaking, enforced deterministically on the way out). As of the
B6 voice pass the same seam also sentence-cases each bubble's lead (core/voice),
so the expected outputs below open capitalized."""
...
def test_hyphen_ranges_survive():
    # number ranges use a hyphen, not an em dash — must NOT be touched (only the
    # lead letter is capitalized; the range hyphens are left alone)
